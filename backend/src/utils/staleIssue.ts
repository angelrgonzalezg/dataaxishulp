...
/**
 * Terminal / closed statuses that must never count as limbo
 * (and are treated as "done" for Monday/Jira open vs closed split).
 */
...
export function countStaleItems(
  items: Array<{ updated_at?: string | null; status?: string | null }>,
  options?: { doneStatus?: string | null; now?: Date },
): number {
  return items.filter((item) =>
    isLimboIssue(item.updated_at, item.status, options),
  ).length;
}
